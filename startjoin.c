...
int SN_Start(SN_Network_descriptor_t* network) {
    ...
    starfishnet_config.nib.tree_branching_factor = network->network_config->routing_tree_branching_factor;
    starfishnet_config.nib.tree_position         = 0;
    starfishnet_config.nib.enable_routing        = 1;
    starfishnet_config.nib.leaf_blocks           = network->network_config->leaf_blocks;
    starfishnet_config.nib.parent_address        = SN_COORDINATOR_ADDRESS;
    memcpy(&starfishnet_config.nib.parent_public_key, &starfishnet_config.device_root_key.public_key, sizeof(starfishnet_config.nib.parent_public_key));
    ...
    starfishnet_config.mib.macShortAddress = SN_COORDINATOR_ADDRESS;
    ...
    starfishnet_config.mib.macPANId = network->pan_id;
...
int SN_Join(SN_Network_descriptor_t* network, bool disable_routing) {
    ...
    if(ret == SN_OK) {
        SN_InfoPrintf("filling NIB...\n");
        starfishnet_config.nib.tree_branching_factor = network->network_config->routing_tree_branching_factor;
        starfishnet_config.nib.tree_position         = network->network_config->routing_tree_position;
        starfishnet_config.nib.enable_routing        = (uint8_t)(disable_routing ? 0 : 1);
        starfishnet_config.nib.leaf_blocks           = network->network_config->leaf_blocks;
        starfishnet_config.nib.parent_address        = network->network_config->router_address;
        memcpy(&starfishnet_config.nib.parent_public_key, &network->network_config->router_public_key, sizeof(starfishnet_config.nib.parent_public_key));
    }
    ...
    if(ret == SN_OK) {
        ...
        if(NETSTACK_RADIO.set_value(RADIO_PARAM_16BIT_ADDR, SN_NO_SHORT_ADDRESS) != RADIO_RESULT_OK) {
            ret = -SN_ERR_RADIO;
        } else {
            starfishnet_config.mib.macShortAddress = SN_NO_SHORT_ADDRESS;
        }
    }
    ...
    if(ret == SN_OK) {
        SN_Table_entry_t parent_table_entry = {
            .short_address = starfishnet_config.nib.parent_address,
            .neighbor      = 1,
            .details_known = 1,
        };
        memcpy(&parent_table_entry.public_key, &starfishnet_config.nib.parent_public_key, sizeof(parent_table_entry.public_key));
    ...
    if(ret == SN_OK) {
        ...
        parent_address.short_address = starfishnet_config.nib.parent_address;
